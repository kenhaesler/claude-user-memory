...
These validations are automated via plugin hooks. `UserPromptExpansion` hooks
run the validation scripts when the `/plan` and `/implement` commands expand
(see `hooks/hooks.json` and `scripts/` in this plugin).

**Validation script contract** (Claude Code hook semantics):
- Exit code 0: Validation passed; stdout is added as context for Claude
- Exit code 2: Validation failed; the expansion is blocked and stderr is fed
  back to Claude so it can fix the defects
