...
description: Active context curation to fight context rot. Curates what goes into the limited context window from the constantly evolving universe of possible information.
---
...
- Built-in `/context` command - inspect where tokens are being spent
- Asking Claude to "optimize context" or "archive completed work"
- Starting a fresh session when switching to a fundamentally different task
...
### Result
- **Large token reduction** (removed API context)
...
**4. User Scope** (`~/.claude/CLAUDE.md` and its imports)
...
Claude Code automatically compacts the conversation when context approaches the window limit. This supplements — but does not replace — manual curation:

- Compaction handles low-level context management automatically in long sessions
- Manual curation remains valuable for keeping high-signal information accessible, archiving patterns to knowledge-core.md, and loading the right context for the current task
- Prefer a fresh session over relying on compaction when switching to a fundamentally different task
